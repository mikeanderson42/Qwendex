...
the base worktree: write events must include `agent_id` and target paths, and a
second active writer is blocked until the first writer reaches a terminal status
or is explicitly closed/tombstoned.
...
Writing hook config is explicit and overwrite-protected. The native ledger and
gate evaluator remain the source of truth if hooks are absent or disabled.
