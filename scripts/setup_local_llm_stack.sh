...
PROMPT_SOURCE=""
if [[ -n "${CODEX_TEXTGEN_SYSTEM_PROMPT_FILE:-}" && -f "$CODEX_TEXTGEN_SYSTEM_PROMPT_FILE" ]]; then
  install -m 0644 "$CODEX_TEXTGEN_SYSTEM_PROMPT_FILE" "$LITELLM_HOME/system_prompt.txt"
  PROMPT_SOURCE="$CODEX_TEXTGEN_SYSTEM_PROMPT_FILE"
elif [[ -f "$LLAMA_CODEX_HOME/runtime/system_prompt.txt" ]]; then
  install -m 0644 "$LLAMA_CODEX_HOME/runtime/system_prompt.txt" "$LITELLM_HOME/system_prompt.txt"
  PROMPT_SOURCE="$LLAMA_CODEX_HOME/runtime/system_prompt.txt"
fi
...
echo "Optional system prompt: ${PROMPT_SOURCE:-none (bridge built-ins only)}"
