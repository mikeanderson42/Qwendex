...
def run_acceptance(run_id: str, output_root: Path) -> dict[str, Any]:
    ...
    if stock_version.returncode or "0.153.1" not in (stock_version.stdout + stock_version.stderr):
        raise InstallAcceptanceError("stock Codex does not match the supported 0.153.1 build contract")
    ...
    with tempfile.TemporaryDirectory(prefix="qwendex-install-acceptance-") as temporary:
        ...
        normal_receipt = {
            **base_receipt(
            ...
            "claim_scope": (
                "tested Linux and Codex 0.153.1 stable-control-file plus "
